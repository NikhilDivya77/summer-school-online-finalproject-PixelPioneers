...
// Function to select exactly 25 questions totaling exactly 100 marks
function getRandomQuestions(questionsData) {
    // First filter out any questions with marks > 100 (though unlikely)
    const validQuestions = questionsData.filter(q => q.marks <= TOTAL_MARKS);
    
    // We'll use a combination of random selection and backtracking
    let bestCombination = [];
    ...
    for (let attempt = 0; attempt < 500; attempt++) {
        ...
        // Track the best combination we've found so far
    ...
    // If we didn't find an exact match, try to adjust the best combination
    if (bestCombination.length === TOTAL_QUESTIONS && closestDiff <= 2) {
        // Try to swap questions to reach exact 100 marks
...
function displayCurrentQuestion() {
    ...
    // Only show penalty for MCQ questions
...
function getImagePath(imageName) {
    ...
    // Handle local image paths (assuming structure: project_root/questionbank/images/)
    // Adjust the relative path based on your actual directory structure
...
function saveAnswer() {
    ...
    // Update the progress display after saving answer
    updateProgress();
}
...
if (nextBtn) {
    nextBtn.addEventListener('click', function() {
        ...
        // Record time before moving to next question
...
if (submitBtn) {
    submitBtn.addEventListener('click', function(e) {
        ...
        // Record time for last question before submitting
        if (questionStartTime !== null) {
            const timeSpent = Math.floor((Date.now() - questionStartTime) / 1000);
            timeSpentPerQuestion[currentQuestionIndex] += timeSpent;
        }
...
// Calculate results with proper penalty rules
...
//             // Apply penalties based on question type
//             if (question.type === "numerical") {
//                 // No penalty for wrong numerical answers
//                 marksObtained = 0;
//             } else if (Array.isArray(question.answer)) {
//                 // No penalty for wrong MSQ answers
//                 marksObtained = 0;
//             } else {
//                 // Only MCQ wrong answers get penalty (1/3 deduction)
//                 marksObtained = -(question.marks / 3);
//             }
//         }
//         // Unattempted questions automatically get 0 marks (no penalty)
...
async function calculateResult() {
    ...
    // Calculate accuracy and attempted count
    ...
    try {
        ...
        const response = await fetch('http://localhost:5000/api/progress/save', {
            ...
            body: JSON.stringify({
                testType: 'Chapter-Mock [Algorithm/Algorithm Analysis]',
                testId,
                score: totalMarks,
...
// Display results with all summary cards
function displayResult(questionResults, totalMarks, totalMaxMarks) {
    // Show sidebar again when results are shown
    ...
    // Setup the review answers feature
    ...
    if (retryBtn) {
        retryBtn.addEventListener('click', () => {
            ...
            // Hide sidebar again if retrying
            const sidebar = document.querySelector('.sidebar');
            if (sidebar) {
...
async function initTest() {
    ...
    // Get questions that sum to exactly 100 marks with exactly 65 questions
    ...
    reviewBtn.addEventListener('click', function() {
        ...
        if (backBtn) {
            backBtn.addEventListener('click', function() {
                ...
                // Restore the original results view
                const resultSummary = document.querySelector('.result-summary');
                if (resultSummary) resultSummary.style.display = 'flex';
                
                // Rebuild the questions grid (if needed)
